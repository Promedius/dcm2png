...
def apply_color_lut(
    ...
    * :dcm:`Supplemental Palette Color LUTs
    ...
    if 'RedPaletteColorLookupTableData' in ds:
        ...
        # NOTE: endianess를 고려하여 바이트 배열을 읽도록 수정
        actual_depth = len(r_lut) / nr_entries * 8
        dtype = np.dtype('{}u{:.0f}'.format('<' if ds.is_little_endian else '>', len(r_lut) / nr_entries))
...
def apply_voi_lut(
    ...
    * DICOM Standard, Part 4, :dcm:`Annex N.2.1.1
    ...
    # NOTE: windowing이 적용 되었는지 여부도 함께 반환
    if valid_voi and valid_windowing:
        if prefer_lut:
            return apply_voi(arr, ds, index), True

        return apply_windowing(arr, ds, index), True
